class Solution:
    def solve(self, board: List[List[str]]) -> None:
        """
        three pass approach:
            go through and mark the regions that are *not* surrounded
            go through and mark the remaining O cells to X
            go through and return the non-surrounded regions to O
        ...
        for i in range(rows):
            for j in range(cols):
                cur = (i, j)
                if cur in visited or cur in captured or board[i][j] == 'X':
                    continue
